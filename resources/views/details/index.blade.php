    ...
    <div class="max-w-screen-xl px-4 py-3 mx-auto md:px-6">
        <div class="flex items-center">
            <ul class="flex flex-row mt-0 mr-6 space-x-8 text-sm font-medium">
                <li>
                    <a href="{{ route('customers.index') }}" class="px-3 py-1 flex space-x-2 mt-5 rounded-md border border-gray-50 cursor-pointer hover:bg-gray-400 hover:border-gray-500 hover:text-gray-50">Customers</a>
                </li>
                <li>
                    <a href="{{ route('details.index') }}" class="bg-gray-500 border-gray-600 text-white px-3 py-1 flex space-x-2 mt-5 rounded-md border border-gray-50 cursor-pointer hover:bg-gray-400 hover:border-gray-500 hover:text-gray-50">Details</a>
                </li>
            </ul>
        </div>
    </div>
            ...
            <div class="p-6">
                ...
                <form method="POST" action="{{ route('details.store') }}">
                    ...
                    <div>
                        ...
                        <input type="text" name="name" class="block w-96 appearance-none rounded-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="district name" required>
                    </div>    
                    <br>
...
<div id="editData" tabindex="-1" class="bg-opacity-50 fixed top-0 left-0 right-0 z-50 hidden p-4 overflow-x-hidden overflow-y-auto md:inset-0 h-modal md:h-full">
    <div class="relative w-full h-full max-w-md md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6">
                <h3 class="mb-5 text-center text-xl font-normal text-gray-700 dark:text-gray-400">District</h3>
                <form method="POST" action="{{ route('details.districtUpdate', 'id', 'name') }}">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="id"  id="edit_id">
                    <div>
                        <label for="name">District Name</label><br>
                        <input type="text" name="name" id="edit_name" class="block w-96 appearance-none rounded-md border border-gray-300 px-3 py-2 text-gray-900 placeholder-gray-500 focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm" placeholder="district name" required>
                    </div>    
                    <br>
    ...
    <div class="relative w-full h-full max-w-md md:h-auto">
        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
            ...
            <div class="p-6 text-center">
                <form method="POST" action="{{ route('details.districtDestroy', 'id') }}">
                    @csrf
                    @method('DELETE')
