<nav class="bg-gray-50 sticky top-0 border-b">
  ...
  <div class="md:hidden" id="mobile-menu">
    <div class="space-y-1 px-2 pt-2 pb-3 sm:px-3">        
      ...
      @endif
      <a href="{{ route('customers.index') }}" class="{{ (request()->segment(1) == 'customers') || (request()->segment(1) == 'details') ? 'bg-emerald-700 border-emerald-700': '' }} px-3 py-1 flex space-x-2 mt-10 rounded-md border border-gray-50 cursor-pointer hover:bg-emerald-600 hover:border-emerald-700">Customers</a>
      <a href="{{ route('items.index') }}" class="{{ (request()->segment(1) == 'items') || (request()->segment(1) == 'itemdetails') ? 'bg-emerald-700 border-emerald-700': '' }} px-3 py-1 flex space-x-2 mt-10 rounded-md border border-gray-50 cursor-pointer hover:bg-emerald-600 hover:border-emerald-700">Items</a>
      <a href="{{ route('invoices.index') }}" class="{{ (request()->segment(1) == 'invoices') ? 'bg-emerald-700 border-emerald-700': '' }} px-3 py-1 flex space-x-2 mt-10 rounded-md border border-gray-50 cursor-pointer hover:bg-emerald-600 hover:border-emerald-700">Invoices</a>
      @if (auth()->user()->type == 'Admin')
      <a href="{{ route('reports.index') }}" class="{{ (request()->segment(1) == 'reports') ? 'bg-emerald-700 border-emerald-700': '' }} px-3 py-1 flex space-x-2 mt-10 rounded-md border border-gray-50 cursor-pointer hover:bg-emerald-600 hover:border-emerald-700">Reports</a>   
      @endif
    </div>
